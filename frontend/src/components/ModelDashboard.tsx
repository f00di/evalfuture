...
export default function ModelDashboard() {
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white shadow-panel">
      <div id="model-preview" className="border-b border-slate-200 p-5 sm:p-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.14em] text-tealFinance">
              Free Initial Comparison
            </p>
            <h3 className="mt-2 text-2xl font-semibold text-navy sm:text-3xl">
              Start with a free comparison
            </h3>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-slateFinance sm:text-base">
              Enter your assumptions below to preview the rent-vs-buy outcome and download a
        ...
        <p className="mt-4 rounded-md border border-goldFinance/30 bg-inputAmber/50 px-3 py-2 text-xs leading-5 text-slateFinance">
          The GitHub Pages version creates an Excel-compatible two-sheet workbook in your
          browser. A backend export can be connected later for a branded formula workbook.
        </p>
      </div>
      ...
      <div className="grid gap-6 p-5 sm:p-6 xl:grid-cols-[minmax(320px,440px)_minmax(0,1fr)]">
        <InputPanel
          form={form}
...
function FieldGroup({
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-creamFinance/70 p-4">
  ...
  return (
    <label className="grid gap-1 text-sm">
      ...
      <input
        ...
        className="rounded-md border border-slate-300 bg-inputAmber px-3 py-2 text-navy outline-none transition focus:border-tealFinance focus:ring-2 focus:ring-tealFinance/20"
      />
    </label>
  );
}
  ...
  return (
    <label className="grid gap-1 text-sm">
      ...
      <input
        ...
        className="numeric min-w-0 rounded-md border border-slate-300 bg-inputAmber px-3 py-2 text-navy outline-none transition focus:border-tealFinance focus:ring-2 focus:ring-tealFinance/20"
      />
    </label>
  );
}
...
function PercentField({
  ...
  return (
    <label className="grid gap-1 text-sm">
      <span className="font-medium text-slateFinance">{label}</span>
      <div className="flex rounded-md border border-slate-300 bg-inputAmber transition focus-within:border-tealFinance focus-within:ring-2 focus-within:ring-tealFinance/20">
        <input
          type="number"
...
function KpiCards({ preview }: { preview: EvaluationPreview | null }) {
  const items = [
    ...
    ["Final options comparison", preview ? money(preview.finalOptionsComparison) : "-"]
  ];
  return (
    <section className="grid gap-4 md:grid-cols-2 2xl:grid-cols-5">
      {items.map(([label, value]) => (
        <div key={label} className="min-h-[118px] rounded-lg border border-slate-200 bg-white p-4">
          <p className="text-sm font-medium text-slateFinance">{label}</p>
          <p className="numeric mt-3 break-words text-xl font-semibold leading-tight text-navy sm:text-2xl">
            {value}
          </p>
        </div>
      ))}
...
function MarketSection({
  ...
  return (
    <section className="grid gap-6 2xl:grid-cols-[minmax(0,1.05fr)_minmax(380px,0.95fr)]">
      <div className="rounded-lg border border-slate-200 bg-white p-5">
        <div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h3 className="text-lg font-semibold text-navy">Market variation</h3>
      ...
      <div className="rounded-lg border border-slate-200 bg-white p-5">
        <h3 className="text-lg font-semibold text-navy">Property Market Price Fluctuations</h3>
        <div className="mt-4 h-[360px] min-h-[360px] min-w-[280px]">
          {hasMounted ? (
...
function MarketTable({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="overflow-hidden rounded-lg border border-slate-200">
      ...
      <div className="scrollbar-soft max-h-[470px] overflow-auto">
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-5">
      <div className="mb-4 flex items-center justify-between gap-4">
        ...
        <span className="shrink-0 text-sm font-semibold text-tealFinance">
          {preview.comparisonRows.length} rows
        </span>
      </div>
      <div className="scrollbar-soft max-h-[520px] overflow-auto">
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-md border border-slate-200 bg-creamFinance px-3 py-2">
      ...
      <p className="numeric mt-1 break-words text-sm font-semibold text-navy">{value}</p>
    </div>
  );
}
